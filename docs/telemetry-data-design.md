...
PostgreSQL + TimescaleDB（生产事实源）
  ├─ 农场 → 生产区域 / 营养液回路 / 种植塔 → 种植孔位
  ├─ 种植周期 → 生产批次 → 植株实例 ↔ 孔位占用关系
  ├─ 采收批次 → 包装批次 → 包装单元 → 订单
  └─ 传感器安装关系 → 测量 / 事件 / 检验 / 合格证明 / 召回
                         ↓ 发布审批与脱敏投影
...
实验只表示研发与验证上下文，可选关联一个或多个生产批次。没有实验的商业生产批次仍然成立；同一个实验也不能替代生产批次、采收批次或包装批次。
...
| 实验记录 | Markdown / PostgreSQL 实验登记 | 作为可选上下文，通过明确关系链接生产批次 |
...
- `published_batch_topology`：批次在有效时段内关联的生产区域、种植塔、营养液回路标签；
...
| `shared_exposure` | 继承生产区域/种植塔/营养液回路的共享环境 | 不得写成某株专属 EC、pH、温湿度 |
| `mass_balance_allocation` | 依据重量或数量守恒分配 | 不得伪装成逐株称量 |
| `derived` | 有版本算法计算的结果 | 必须能追到输入范围和规则版本 |
| `unknown` | 无法可靠确定 | 不得用 0 或设计值填补 |

营养液 EC、pH、液温通常属于营养液回路或具体安装点；环境温湿度、PAR/DLI 通常属于生产区域或覆盖范围。植株实例页面若展示这些数据，只能标为“共享暴露”。
...
本阶段只上线代码层面的中英文静态生产批次追溯页：

- 展示批次身份、农场、作物、生产周期、关联塔/区域/营养液回路；
- 分开显示生产事件、共享测量、检测、采收批次和证明；
- 空集合明确显示“尚无已审核记录”；
- 实验链接仅作为可选研发上下文；
- 不提供二维码访问令牌、订单查询、单株公开页、召回执行后台或写入接口。
